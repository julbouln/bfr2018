...
public:
    ...
    void SetEntitiesQueue(std::string key, std::queue<EntityID> value)  { entitiesQueues[key] = value; }
    std::queue<EntityID> &GetEntitiesQueue(std::string key)
    {
        if (entitiesQueues.find(key) == entitiesQueues.end()) {
            entitiesQueues[key] = std::queue<EntityID>();
        }
        return entitiesQueues[key];
    }
    bool HasEntitiesQueue(std::string key) const  { return entitiesQueues.find(key) != entitiesQueues.end(); }
...
protected:
    ...
    std::unordered_map<std::string, std::queue<EntityID>> entitiesQueues;
